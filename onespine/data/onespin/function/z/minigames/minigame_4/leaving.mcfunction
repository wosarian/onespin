...
$execute \ 
    if entity @s[scores={leaveSurvival=1,OSID=$(ID)},predicate=!onespin:location/dimension/any_survival] \ 
...
$execute as @s[scores={leaveSurvival=1,OSID=$(ID)},predicate=onespin:location/dimension/any_survival] run tag @s remove OS-4-locked
$execute as @s[scores={leaveSurvival=1,OSID=$(ID)},predicate=onespin:location/dimension/any_survival] run gamemode adventure @s
$execute as @s[scores={leaveSurvival=1,OSID=$(ID)}] in onespin:dimension_2 run tp @s -269 57 49
$execute as @s[scores={leaveSurvival=1,OSID=$(ID)}] at @s unless predicate onespin:location/dimension/any_survival in onespin:dimension_2 run spawnpoint @s -269 57 49
$execute as @s[scores={leaveSurvival=1,OSID=$(ID)}] run tag @s add OS-4-lockedOut
$execute as @s[scores={leaveSurvival=1,OSID=$(ID)}] at @s unless predicate onespin:location/dimension/any_survival run scoreboard players set @s leaveSurvival 0