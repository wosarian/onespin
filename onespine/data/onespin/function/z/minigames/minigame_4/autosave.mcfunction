$execute as @s[tag=!OS-4-lockedOut,tag=!OS-4-joining,scores={leaveSurvival=0,OS-playerID=$(ID)}] at @s \ 
    if predicate onespin:location/in_dimension/any_minecraft \
...
$execute as @s[tag=!OS-4-lockedOut,tag=!OS-4-joining,scores={leaveSurvival=0,OS-playerID=$(ID)}] at @s \
    if predicate onespin:location/in_dimension/overworld \
